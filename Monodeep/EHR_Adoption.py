...
df['Pain Score'] = np.where(df['Pain Score'].notnull(),"YES", "NO")
df['Care Pathway'] = np.where(df['Care Pathway'].notnull(),"YES", "NO")

df['NextFollowUpCM'] = np.where(df['NextFollowUpCM'].notnull(),"YES", "NO")


df_csv = df[["Patient Id", "Patient Name", "Phone", "Clinic Name", "Consult Date", "Consult Time", "Patient Attendance Status",  "Care Pathway","NextFollowUpCM","Name",'Pain Score']]

print(df_csv)


df_csv.to_csv('EHR_Adoption.csv',index=False)
...
spreadsheetId = '1aBkjro4oifrCVz6vzxZigM4BZ60wR4IUWP2NIIPhWDU' 

sheetName = 'Raw'        # Please set sheet name you want to put the CSV data.
csvFile = 'EHR_Adoption.csv'  # Please set the filename and path of csv file.
