...
sheet = client.open_by_key("1PLaCbq0pVSLcxJu5aiwhtQuP_P_9UyJLGdjUor3BJcc") # Open by key the spreadhseet
#sheet.share
tab = sheet.worksheet('Visit')
calls = pd.DataFrame(tab.get_all_records())
df = calls[['Full Name','Scheduled By','Speciality','hot','warm','cold','Date','Nthvisit','Month','Doctor Status On Nivaan What Is The Business Possibility With The Doctor' ]]
...
latest_status = df.groupby('Full Name')['Doctor Status On Nivaan What Is The Business Possibility With The Doctor'].last().reset_index()

# Merge the latest_status DataFrame with the original DataFrame on 'Full Name'
df = pd.merge(df, latest_status, on='Full Name', how='left')
df_Rename = df.rename(columns={'Doctor Status On Nivaan What Is The Business Possibility With The Doctor':'LatestStatus'})

print(df_Rename)

#Group by Doctor and calculate Ageing
current_date = datetime.now()
df_Rename['Ageing'] = (current_date - df.groupby('Full Name')['Date'].transform('min')).dt.days

df_Rename['Visits'] = df.groupby('Full Name')['Nthvisit'].transform('max')
df_Rename['Visits_Month'] = df.groupby(['Full Name', 'Month'])['Nthvisit'].transform('count')
print(df_Rename)
df_NEW = df_Rename.rename(columns={'Full Name': 'Doctor_Name', 'Scheduled By': 'DRO_Name'})
...
# DF_FINAL_DATA = unique[["Doctor_Name", "DRO_Name", "Speciality", "LatestStatus", "Ageing", "Visits","Visit", "Leads", "OPDs", "OPD%", "OPD Amt", "CRP Booked", "CRP%", "CRP Amt", "PRC Done", "PRC%", "PRC Amt", "Total Revenue", "Dr. Payout",
#   "Aug_Visits", "Sep_Visits", "Oct_Visits", "Nov_Visits", "Dec_Visits", "Jan_Visits", "Feb_Visits", "Mar_Visits"]]
